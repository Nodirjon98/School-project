/**
 * Zero-dependency Web Audio API sound synthesizer for Vocab Contest
 * Works 100% offline and in all sandboxed environments
 */
...
export function playSoundVictory(): void {
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
